...
from liaa.node import Node, PingNode, IndexNode, GenericNode
...
class RPCFindResponse:
    def __init__(self, response: ResponseIndexItem):
        """
		RPCFindResponse
    ...
    def get_value(self) -> Optional[bytes]:
        ...
        if isinstance(self.response[1], dict):
            key = list(self.response[1].keys())[0]
            return self.response[1][key]
        return None

    def item(self) -> Union[PingNodeInfo, IndexNodeAsDict]:
        return self.response[1]

    def has_response_item(self) -> bool:
        return self.did_happen() and len(self.response) == 2

    def get_node_list(self) -> Optional[List[PingNode]]:
        """
		...
		"""
        if isinstance(self.response[1], list):
            key, *_ = self.response[1]
            return [PingNode(key)]
        return None
...
class KademliaProtocol(RPCDatagramProtocol):
    ...
    def get_refresh_ids(self) -> List[str]:
        """
        ...
        for bucket in self.router.lonely_buckets():
            node = random.choice(bucket.get_set())
            ids.append(node.key)
        return ids

    def rpc_stun(self, sender: PingNode) -> PingNode:
        """
		Execute a S.T.U.GenericNode procedure on a given sender
    ...
    async def call_find_node(
		...
		"""
        result = await self.find_node(to_ask.addr(), self.source_node.key, to_find.key)
    ...
    async def call_find_value(
		...
		"""
        result = await self.find_value(to_ask.addr(), self.source_node.key, to_find.key)
        return self.handle_call_response(result, to_ask)
    ...
    async def call_store(
        self, to_ask: PingNode, to_store: IndexNode
    ) -> ResponseIndexItem:
        """
    ...
    def welcome_if_new(self, node: PingNode) -> None:
        ...
        for inode in self.storage:
            ...
            if not neighbors or (is_closer_than_furthest and curr_distance_to_new):
                asyncio.ensure_future(self.call_store(node, inode))
