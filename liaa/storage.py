...
class BaseStorage(abc.ABC):
	...
	@abc.abstractmethod
	def get(self, key: Any, default: Optional[StorageNode]) -> Optional[StorageNode]:
...
class EphemeralStorage(BaseStorage):
	def __init__(self, node: NetworkNode, ttl: int = 604800):
		...
		self.data: Dict[int, StorageNode] = OrderedDict()

	def get(
		self, key: int, default: Optional[StorageNode] = None
	...
	def remove(self, node: Node):
		...
		if node in self:
			del self.data[node.long_id]

	def _triple_iter(self) -> List[Tuple[int, StorageNode]]:
		"""
	...
	def __contains__(self, node: Node) -> bool:
		return node.long_id in self.data
