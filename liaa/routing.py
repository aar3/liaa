...
class KBucket:
	...
	def remove_node(self, node):
		if node.digest in self.replacement_nodes:
			del self.replacement_nodes[node.digest]

		if node.digest in self.nodes:
	...
	def is_new_node(self, node):
		return node.digest not in self.nodes
	...
	def add_node(self, node):
		...
		if node.digest in self.nodes:
			...
			if node.digest in self.replacement_nodes:
				del self.replacement_nodes[node.digest]
			self.replacement_nodes[node.digest] = node
			return False
		return True
